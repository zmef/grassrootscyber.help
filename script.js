...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function renderQuestion(index) {
        ...
        const optionsHtml = q.options.map((opt, i) => `
            <label class="option-label">
                <input type="radio" name="question${index}" value="${i}" ${userAnswers[index] === i ? 'checked' : ''} required>
                ${opt}
            </label>
        `).join('');
        wellnessForm.innerHTML = `
            <div class="question-block">
                <h3>${q.question}</h3>
                <div class="options-group">${optionsHtml}</div>
            </div>
            <div class="navigation-buttons">
                <button type="button" id="prevBtn" class="btn secondary-btn${index === 0 ? ' hidden' : ''}">
                    <i class="fas fa-arrow-left"></i> Previous
                </button>
                <button type="button" id="nextBtn" class="btn primary-btn${index === questions.length - 1 ? ' hidden' : ''}">
                    <i class="fas fa-arrow-right"></i> Next
                </button>
                <button type="submit" id="submitBtn" class="btn primary-btn${index === questions.length - 1 ? '' : ' hidden'}">
                    <i class="fas fa-check-circle"></i> Get Report
                </button>
            </div>
        `;
        // Update progress
    ...
    function saveAnswer() {
        const radios = wellnessForm.querySelectorAll('input[type="radio"]');
        let selected = null;
    ...
    // Prevent form submit on Enter key (to avoid accidental submits)
    if (wellnessForm) {
        wellnessForm.addEventListener('submit', handleSubmit);
    }
});
